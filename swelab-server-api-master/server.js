...
app.post('/register',function(req,res){
  ...
  mongodb['cores'].db.collection('user').insert({username:user,password:encrypt(pass)},function(err,data){
    if(!err){
      res.send({data:data,msg:"ins"})
   }else{
     res.send({msg:"can't ins"})
   }
  })
})
...
app.post('/login',function(req,res){
  ...
  mongodb['cores'].db.collection('users').find({username:user,password:encrypt(pass),function(err,data){
    if(!err){
      if(data.length>0){
